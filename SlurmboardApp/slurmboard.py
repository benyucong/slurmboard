...
def collect_storage_quota():
    ...
    try:
        ...
        return {
            "generated_at": time.strftime("%Y-%m-%d %H:%M:%S"),
            "supported": True, "source": "custom", "rows": [],
            "raw": None, "error": str(exc),
        }
    if not candidates:
        return {
            "generated_at": time.strftime("%Y-%m-%d %H:%M:%S"),
            "supported": False, "source": None, "rows": [], "raw": None,
    ...
    for source, command in candidates:
        ...
        if rows:
            return {
                "generated_at": time.strftime("%Y-%m-%d %H:%M:%S"),
                "supported": True, "source": source, "rows": rows,
                "raw": None, "error": None,
            }
    ...
    if first_raw:
        return {
            "generated_at": time.strftime("%Y-%m-%d %H:%M:%S"),
            "supported": True, "source": first_raw[0], "rows": [],
            "raw": first_raw[1], "error": None,
        }
    return {
        "generated_at": time.strftime("%Y-%m-%d %H:%M:%S"),
        "supported": True, "source": candidates[0][0], "rows": [], "raw": None,
...
def build_cluster_summary(refresh=False):
    def load():
        ...
        # Always collect the compact per-node resource snapshot for aggregate
        # allocation figures.  This query does not expose or render the node
        # rows in the initial page, so large clusters can still show exact GPU
        # and memory totals without paying the DOM cost of node details.
...
def build_snapshot():
    """Build the initial page with the current user's lightweight job panels."""
  ...
  <div class="three-col">
    ...
    <aside class="left-col">
      ...
      <section id="quota-section">
        ...
        <div id="quota-panel"><p class="muted" style="font-size:12px">Checking quota support…</p></div>
      </section>
    </aside>
...
function renderStorageQuota() {
  ...
  section.hidden = QUOTA_DATA?.supported === false;
  if (section.hidden) return;
  if (!QUOTA_DATA) {
    const prompt = document.createElement('p');
    prompt.className = 'muted';
    prompt.style.fontSize = '12px';
    prompt.textContent = 'Checking quota support…';
...
function renderNodeSubTable(container, partName) {
  clearIncrementalTable(container);
  const partition = SNAPSHOT.partitions.find(p => p.name === partName);
  if (partition && !partition.nodes_loaded) {
    container.innerHTML = partition.detail_error
      ? '<div style="padding:10px;color:var(--bad)">Could not load node details for this partition.</div>'
      : '<div style="padding:10px;color:var(--muted)">Loading node details for this partition…</div>';
    return;
  }
  ...
  if (!nodes.length) {
    container.innerHTML = '<div style="padding:10px;color:var(--muted)">No nodes in this partition.</div>';
    return;
  }
  ...
  container.innerHTML = `<div class="inner-wrap"><table class="inner-table">
    <thead><tr>
  ...
  renderIncrementalRows(container, container.querySelector('tbody'), nodes, rowHtml,
                        container.closest('.center-col'));
}

// ── incremental job-table rendering ────────────────────────────────────
const JOB_BATCH_SIZE = 100;
const incrementalCleanups = new WeakMap();

function clearIncrementalTable(container) {
  const cleanup = incrementalCleanups.get(container);
  if (cleanup) cleanup();
  incrementalCleanups.delete(container);
}

function renderIncrementalRows(container, tbody, rows, rowHtml, scrollRoot) {
  clearIncrementalTable(container);
  let rendered = 0;
  const sentinel = document.createElement('tr');
  sentinel.innerHTML = '<td style="height:1px;padding:0;border:0"></td>';
  tbody.appendChild(sentinel);
  let observer;
  const appendBatch = () => {
    const end = Math.min(rendered + JOB_BATCH_SIZE, rows.length);
    if (end > rendered) {
      sentinel.insertAdjacentHTML('beforebegin', rows.slice(rendered, end).map(rowHtml).join(''));
      rendered = end;
    }
  ...
  incrementalCleanups.set(container, () => observer.disconnect());
  appendBatch();
  if (sentinel.isConnected) observer.observe(sentinel);
}

function renderPartitionJobTable(container, jobs, isPending) {
  clearIncrementalTable(container);
  if (!jobs.length) {
    container.innerHTML = '<div style="padding:8px 0;color:var(--muted);font-size:13px">No jobs.</div>';
    return;
  }
  ...
  renderIncrementalRows(container, container.querySelector('tbody'), jobs, rowHtml,
                        container.closest('.center-col'));
}
...
function buildPartCells(p) {
  ...
  const vramCell = p.gpu_vram_gb != null
    ...
    : '<span class="muted">—</span>';
  const jobsCell = `<span class="job-toggle" data-kind="running"
...
function patchPartRow(partName) {
  ...
  if (expandTr && expandTr.classList.contains('nodes-expand-row') && cur) {
    ...
    if (td) {
      clearIncrementalTable(td);
      if (cur === 'nodes') {
        renderNodeSubTable(td, partName);
      } else {
        const jobs = (p.jobs || []).filter(j => j.state === cur.toUpperCase());
        renderPartitionJobTable(td, jobs, cur === 'pending');
      }
    }
  }
}
...
function renderPartitions() {
  ...
  tbody.querySelectorAll('.nodes-expand-row > td').forEach(clearIncrementalTable);
  tbody.innerHTML = '';
...
function renderActiveQueue() {
  ...
  renderIncrementalRows(panel, panel.querySelector('tbody'), jobs, j => jobRow(j, false), panel);
...
function renderHistoryJobs() {
  ...
  renderIncrementalRows(panel, panel.querySelector('tbody'), allJobs, j => jobRow(j, false), panel);
